...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/12/14 16:35:13 by mvogee            #+#    #+#             */
/*   Updated: 2016/12/14 16:35:14 by mvogee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*get_output(char *mods, va_list arglist, int precision)
{
	...
	if (len == L)
		...
		ret = pf_itoa_base((unsigned char)va_arg(arglist, unsigned long), 8);
	else if (len == H)
		ret = pf_itoa_base((unsigned short)va_arg(arglist, unsigned long), 8);
	else
		ret = pf_itoa_base((unsigned int)va_arg(arglist, unsigned int), 8);
	if (!ret)
		return (NULL);
