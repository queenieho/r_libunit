...
static char	*get_output(char *mods, va_list arglist, int precision)
{
	...
	if (len == L)
		...
		ret = pf_utoa((unsigned char)va_arg(arglist, unsigned long));
	else if (len == H)
		ret = pf_utoa((unsigned short)va_arg(arglist, unsigned long));
	else
		ret = pf_utoa((unsigned int)va_arg(arglist, unsigned int));
	if (!ret)
		return (NULL);
